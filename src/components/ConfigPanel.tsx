...
import { LifestyleConfig, GenerationEngine } from "../types";
import { 
  Sparkles, Sliders, Image, Type, Compass, 
  Lightbulb, Square, Layout, Maximize2 
} from "lucide-react";
          ...
          <div className="grid grid-cols-2 gap-2 max-h-[140px] overflow-y-auto pr-1 bg-brand-100/15 p-1 rounded-xl border border-brand-100/50" id="scene-preset-grid">
            {SCENE_PRESETS.map((preset) => (
              <button
                ...
                className={`flex items-center gap-2 p-2 rounded-lg text-left border transition-all text-xs cursor-pointer truncate ${
          ...
          <div className="space-y-1.5 max-h-[110px] overflow-y-auto p-1 bg-brand-100/10 rounded-xl border border-brand-100/60" id="lighting-profile-selector">
            {LIGHTING_PROFILES.map((profile) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-2 rounded-lg text-left border transition-all text-xs cursor-pointer ${
                  config.lighting === profile.name
